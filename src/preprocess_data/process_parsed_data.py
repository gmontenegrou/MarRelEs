...
import stanza
import argparse
from process_other_feat import process_phrases

stanza.download("es")

nlp = stanza.Pipeline(
    "es", processors="tokenize,pos,ner,lemma,depparse", tokenize_pretokenized=False
)
...
def process_text_with_stanza(dataframe, text_column):
    ...
    for text in dataframe[text_column]:
        ...
        dep = [
            (word.text, word.head, word.deprel)
            for sentence in doc.sentences
            for word in sentence.words
        ]
...
if __name__ == "__main__":
    ...
    xparser.add_argument(
        "text_column", help="Column name to the text column affected by the parsing"
    )
    args = xparser.parse_args()
    # function use
    df = pd.read_csv(args.data)
    parsed_data = process_text_with_stanza(df, args.text_column)
    cms = list(parsed_data["CM"])
    contexts = list(parsed_data["context"])
    all_phrases, noise_detected = process_phrases(contexts, cms)
    parsed_data["sent_context"] = all_phrases
