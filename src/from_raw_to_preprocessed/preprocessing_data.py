## This script allows to preprocess a data folder from the CT-IULA to get three different df 
...
if __name__ == "__main__":
    ...
    combined_df_sorted.to_csv("data/cause_combined_data.csv", index=False)
    print("all data phrases extracted", len(combined_df_sorted))
    neg_filter = token_filter(combined_df_sorted, "sentence")
    neg_filter[0].to_csv("data/cause_combined_data_filtered.csv", index=False)
    print("data without neg", len(neg_filter[0]))
    neg_filter[1].to_csv("data/cause_combined_data_negative.csv", index=False)
